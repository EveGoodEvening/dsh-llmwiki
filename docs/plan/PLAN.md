...
3. **C16 — Add deterministic recovery catalogs and complete structural lint** (`GAP-CATALOG`, `DEF-CANONICAL-LINT`) under §14.3, receiving `src/lint.ts` from C15; draft commit `6516d17` and review-fix commit `e50f45b` are recorded, and verified uncommitted final service/test fixes are complete, awaiting independent static re-review and a final commit.
...
C16 implementation, review fixes, and verification gates are complete. Draft commit `6516d17` has actual subject `feat(catalog): add deterministic wiki listings` and exactly 18 paths: `README.md`, `docs/plan/CHECKLIST.md`, `docs/plan/PLAN.md`, `examples/README.md`, `scripts/check-determinism.ts`, `scripts/smoke.ts`, `src/errors.ts`, `src/lint.ts`, `src/presentation.ts`, `src/prompt.ts`, `src/service.ts`, `src/tools.ts`, `src/types.ts`, `tests/built-package.e2e.spec.ts`, `tests/lint.spec.ts`, `tests/loader.e2e.spec.ts`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`. Review-fix commit `e50f45b` has actual subject `fix(catalog): validate complete listings` and exactly eight paths: `README.md`, `docs/plan/CHECKLIST.md`, `docs/plan/PLAN.md`, `src/service.ts`, `tests/built-package.e2e.spec.ts`, `tests/loader.e2e.spec.ts`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`. Current verified but uncommitted final changes in `src/service.ts` and `tests/service.spec.ts` implement bounded traversal with one directory handle at a time; local ownership and cleanup of `Dir`/`FileHandle` resources; $O(N)$ metadata work plus one page of content bytes; Linux, macOS, and BSD descriptor aliases with fail-closed behavior on unsupported platforms; source error classification; and a self-contained declaration-platform type. Focused service/lint/plugin verification passed 113/113; `pnpm run typecheck`, `pnpm run lint`, and `pnpm run build` passed; Loader plus packed/profile E2E passed 8/8; the dependency audit reported no known vulnerabilities; and no package or lockfile changed. `GAP-CATALOG` and `DEF-CANONICAL-LINT` are implemented and gate-verified; independent static re-review and the final commit remain open. C17–C19B remain open, `GAP-SCHEMA` remains explicitly unresolved, and C19B remains externally blocked.
