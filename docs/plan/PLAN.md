...
- **Decision:** close as by design only after the fixed-root contract is made explicit, regression-protected, and the bundled profile no longer activates policy-exempt host storage without explicit operator opt-in. Do not change runtime repository selection.
...
- **Closure rationale:** close as `not_planned`/by design only after C21 verification and C22 split review confirm documentation, tests, secure activation default, and closure wording agree. The closure comment must cite fixed-root adapter, sequential same-root durable-sharing, and distinct-root isolation evidence; distinguish fixed-root identity from activation-local state; and warn against shared-root concurrent writers and multi-tenant use.
...
- **Decision:** keep host storage outside `ctx.fs`, but do not ship it silently inside a read-only-capable profile. C21 must remove unconditional bundled activation and require explicit operator opt-in/configuration for this policy-exempt store before either issue closes. Do not migrate product I/O to `ctx.fs` in this issue lane.
...
- **Closure rationale:** close as `not_planned`/by design only after C21 verification and C22 split review confirm the warning, exact initialization-capable read/search semantics, conditional search publication, proven OS denial or explicit unsupported/blocked result, and explicit opt-in default are accurate. The closure comment must carry those distinctions, state that issue closure is separately authorization-gated, and state that deployments requiring DSH-enforced read-only behavior, remote providers, or tenant isolation must not opt in to this plugin in that execution boundary.
...
1. **C20 — Freeze issues #2/#3 adjudication:** update only the durable PLAN and CHECKLIST with the decisions, root/identity/policy/compatibility contracts, exact chunk sequence, and C19B separation. Planning commit `d0eb13d` changed exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md` and was review-clean. The first accounting attempt, `b1908d7`, changed only `docs/plan/CHECKLIST.md`; it is durably recorded as incomplete because it did not satisfy the required two-artifact path contract. A future corrective C20 accounting commit must change exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`, record the prior hashes/path sets and disposition, and must not record its own hash; that hash is recorded only by a later accounting commit.
2. **C21 — Publish and regression-protect the host-store contract:** update public README upgrade/release guidance, example/configuration wording, the bundled profile activation, and the existing documentation audit; retain the constructor-root regression; add the explicit tool/command/direct-caller matrix, a sequential two-activation same-root durable-sharing/remount regression, and distinct-root isolation as its negative control; and add only an actually composed sandbox/read-only controlled-denial test plus absent, incomplete, initialized/fresh-index, and initialized/missing-or-stale-index OS-read-only-root behavior. Each OS-read-only fixture must first prove direct Node sentinel denial under the same executing identity or record the gate unsupported/blocked. Precisely preserve the current split: status/list/lint are strictly non-mutating; `readSource`, `readPage`, and search are initialization-capable and read-only-usable only after full initialization; search is also conditionally index-publishing and needs a fresh index to remain read-only. Product source, public APIs, dependencies, storage bytes, and root defaults remain unchanged; no acquire-only read rewrite is planned. If no executable composed DSH denial mechanism exists in the pinned runtime, record that limitation and omit rather than fake that comparison test.
3. **C22a — Durable pre-closure review:** conduct separate scope/isolation and filesystem/security-policy reviews, resolve every finding, rerun affected gates, perform a non-mutating GitHub capability/authorization preflight, approve separate closure comments, and commit the reviewed pre-closure evidence. A successful-authorization pre-closure commit must exist before any issue mutation.
...
Current issue-lane ledger: `d0eb13d` is the valid C20 planning commit with exact paths `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`; `b1908d7` is an incomplete C20 accounting attempt with exact path `docs/plan/CHECKLIST.md`; and `81f63bd` is the C21 implementation commit with exactly `README.md`, `cordis.patch.yml`, `docs/plan/CHECKLIST.md`, `examples/README.md`, `scripts/check-determinism.ts`, `tests/built-package.e2e.spec.ts`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`. The inclusion of `docs/plan/CHECKLIST.md` in `81f63bd` deviated from C21's accounting-only tracker ownership rule; the ledger records the actual path rather than rewriting history. Neither C20 nor C21 is acceptance-complete until the future two-artifact C20 corrective accounting commit and the subsequent C21 review-fix/accounting commit exist and are recorded acyclically without self-hashes.
