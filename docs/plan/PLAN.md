...
3. **C22a — Durable pre-closure review:** in progress. The first split pre-closure review returned **CLEAN**. The second found that the captured-root adapter regression asserted command success/output using a source ID learned from the preceding tool result, so it did not independently pin the captured-root source identity. Commit `9c7cf07` (`test: prove command adapters use captured root`) changed exactly `tests/plugin.spec.ts`, made both command adapters assert the captured repository's exact status/lint output, and passed `pnpm exec vitest run tests/plugin.spec.ts -t "captured root"` 1/1 plus eslint and typecheck. Follow-up commit `e9f504f` (`test: pin captured-root source identity`) changed exactly `tests/plugin.spec.ts`, replaced the tool-derived ID with the deterministic expected source ID across tool, command, and direct-service assertions, and passed the same focused command 1/1 plus eslint and typecheck. The second review finding is fixed and its follow-up disposition is recorded; the final independent **CLEAN** re-review remains pending. GitHub preflight, approved comments, and the reviewed pre-closure commit also remain pending, and no issue mutation is authorized yet.
