...
1. **C20 — Freeze issues #2/#3 adjudication:** update only the durable PLAN and CHECKLIST with the decisions, root/identity/policy/compatibility contracts, exact chunk sequence, and C19B separation. Planning commit `d0eb13d` changed exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md` and was review-clean. The first accounting attempt, `b1908d7`, changed only `docs/plan/CHECKLIST.md`; it is durably recorded as incomplete because it did not satisfy the required two-artifact path contract. The current corrective C20 accounting commit changes exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`, records the prior hashes/path sets and disposition, and intentionally does not record its own hash; that hash remains pending for a later accounting commit under the no-self-hash rule.
...
Current issue-lane ledger: `d0eb13d` is the valid C20 planning commit with exact paths `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`; `b1908d7` is an incomplete C20 accounting attempt with exact path `docs/plan/CHECKLIST.md`; the current exact-two-artifact C20 corrective accounting commit changes only `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`, with its own hash intentionally pending later no-self-hash accounting; `81f63bd` is the C21 implementation commit with exactly `README.md`, `cordis.patch.yml`, `docs/plan/CHECKLIST.md`, `examples/README.md`, `scripts/check-determinism.ts`, `tests/built-package.e2e.spec.ts`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`; `fb579f2` is the C21 review-fix commit with exactly `docs/plan/PLAN.md`, `docs/plan/CHECKLIST.md`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`; and `164bed6` is the Linux-scope review-fix commit with exactly `tests/service.spec.ts`. The inclusion of `docs/plan/CHECKLIST.md` in `81f63bd` deviated from C21's accounting-only tracker ownership rule; the ledger records the actual path rather than rewriting history. C20 and C21 final acceptance remain pending until the current corrective accounting commit hash is recorded and the final independent re-review returns **CLEAN**.
