...
3. **C22a — Durable pre-closure review:** repository-local work complete before GitHub preflight. Review A (scope/root/adapter) found only that the captured-root command regression asserted success for `/wiki status` and `/wiki lint`, which did not prove that either command adapter used the activation-captured root. Commit `9c7cf07` (`test: prove command adapters use captured root`) is the direct fix for that finding: it changed exactly `tests/plugin.spec.ts`, made both command adapters assert the captured repository's exact status/lint output, and passed `pnpm exec vitest run tests/plugin.spec.ts -t "captured root"` 1/1 plus eslint and typecheck. A distinct later adversarial command re-review required the source identity itself to be independently pinned rather than learned from the preceding tool result; this was follow-up hardening, not an additional initial Review A finding. Commit `e9f504f` (`test: pin captured-root source identity`) changed exactly `tests/plugin.spec.ts`, replaced the tool-derived ID with the deterministic expected SHA-256 source ID across tool, command, and direct-service assertions, and passed the same focused command 1/1 plus eslint and typecheck. The corrected attribution and every finding disposition are recorded. Review B (filesystem/permission/security) independently returned **CLEAN** with no findings after inspecting the direct Node I/O boundary, empty bundled patch and explicit opt-in, composed-policy controlled denial, Linux OS-read-only coverage, initialization-capable reads/search, conditional index publication, cleanup, and C19B separation. The final split and aggregate independent re-reviews were unanimous **CLEAN**, with zero unresolved findings. The approved exact issue #2 and issue #3 closure comments remain fixed below with SHA-256 digests `3eed98911b6c5e9b1aa7277b086c748a7f296367d488e758caffd977c0f431a0` and `3ce1a464ae939bb9800c73460528110b81dfd64e73b0a61bc5210c841112a6ae`. Final observed repository-local gates passed exactly: `pnpm run lint`; `pnpm run typecheck`; `pnpm test` (7 files/248 tests); `pnpm run test:e2e` (2 files/9 tests); `pnpm run build && pnpm run check:determinism` with SHA-256 `f4b636ac0401093955bf3938ec8616014da149dc30b5707a9980cac82781fd12`; `pnpm pack --dry-run` producing `evegoodevening-dsh-llmwiki-0.1.2.tgz`; and `git diff --check`. LSP diagnostics were unavailable because no language server was found for `tests/plugin.spec.ts`. The two-path pre-closure commit is prepared for exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`, with its hash pending self-commit under the no-self-hash rule. Non-mutating GitHub issue state/identity/permission/close-reason preflight, posting/closing, resulting URLs/states, partial-state accounting if needed, and final closure accounting remain pending and unchecked; no issue mutation is authorized yet.
4. **C22b — Resumable external mutation and durable accounting:** only an authenticated identity confirmed by the durable successful preflight may act. For each issue, preserve two exact idempotent milestones: approved-comment presence with its URL, then closed state with the supported reason. The approved comment must be confirmed present before attempting that issue's close. After any comment or close failure, immediately inventory both milestones for both issues, then create a partial-mutation accounting commit recording every confirmed action, URL/state, and the exact remaining blocker; a close failure after comment success and a comment failure before close remain distinct resume states. A resumed authorized continuation must consume that durable inventory, re-inventory GitHub, skip completed milestones, and attempt only the missing milestone in order; an issue already closed by another actor but missing its approved comment remains partial and requires only that comment before ledger completion. Close each decision ID only after both its approved comment and supported-reason closure are confirmed. Once both issues satisfy both milestones, create a later final closure-accounting commit with the resulting URLs/states and complete C20–C22 commit/path ledger. If preflight blocks before mutation, record durable `BLOCKED-GITHUB-CLOSURE`, finish repository-local review work, and leave both issues open without fabricated evidence. Any later continuation from that state must rerun the non-mutating preflight and create a new durable successful-authorization/pre-closure update commit before mutation; unchanged passed repository-local reviews may be reused.
...
Current issue-lane ledger: `d0eb13d` is the valid C20 planning commit with exact paths `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`; `b1908d7` is the incomplete C20 accounting attempt with exact path `docs/plan/CHECKLIST.md`; `c311a1b` is the exact-two-artifact corrective C20 accounting commit with exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`, completing C20 accounting; `81f63bd` is the C21 implementation commit with exactly `README.md`, `cordis.patch.yml`, `docs/plan/CHECKLIST.md`, `examples/README.md`, `scripts/check-determinism.ts`, `tests/built-package.e2e.spec.ts`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`; `fb579f2` is the C21 review-fix commit with exactly `docs/plan/PLAN.md`, `docs/plan/CHECKLIST.md`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`; and `164bed6` is the Linux-scope review-fix commit with exactly `tests/service.spec.ts`. The inclusion of `docs/plan/CHECKLIST.md` in `81f63bd` deviated from C21's accounting-only tracker ownership rule; the ledger records the actual path rather than rewriting history. Three independent final C21 re-reviews unanimously returned **CLEAN** with no findings, so all C21 implementation, verification, review, commit, and accounting work is complete. C22 Review A's sole initial finding—weak captured-root command status/lint assertions—was directly fixed by `9c7cf07` (`test: prove command adapters use captured root`), which changed exactly `tests/plugin.spec.ts`; the distinct later requirement to independently pin the SHA-256 source ID was adversarial re-review hardening, not another initial Review A finding, and was completed by `e9f504f` (`test: pin captured-root source identity`), also changing exactly `tests/plugin.spec.ts`. Both passed the focused 1/1 test plus eslint and typecheck. Commit `2b34007` (`docs(plan): record clean host-store contract review`) and commit `0922a96` (`docs(plan): account command adapter review fixes`) each changed exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`. Review B returned **CLEAN** with no findings, and the final split/aggregate independent C22 re-reviews were unanimously **CLEAN** with zero unresolved findings. The approved issue comments and their exact digests are recorded above. Final repository-local verification passed: lint, typecheck, 7 files/248 unit tests, 2 files/9 E2E tests, build plus determinism SHA-256 `f4b636ac0401093955bf3938ec8616014da149dc30b5707a9980cac82781fd12`, dry-run pack artifact `evegoodevening-dsh-llmwiki-0.1.2.tgz`, and `git diff --check`; LSP diagnostics were unavailable because no language server was found for `tests/plugin.spec.ts`. The pre-closure commit row is prepared for exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`, with hash pending self-commit. `ISSUE-2-WORKSPACE-SCOPE` and `ISSUE-3-FILESYSTEM-POLICY` are repository-resolved with no runtime fix required, but neither GitHub issue is closed; GitHub state/identity/permission/close-reason preflight, comments, closure, URLs/states, partial accounting if needed, and final closure accounting remain unchecked. C19B remains externally blocked and `GAP-SCHEMA` remains intentionally unresolved.
