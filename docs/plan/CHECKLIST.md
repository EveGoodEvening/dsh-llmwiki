...
**Status:** planning commit `d0eb13d` is complete and review-clean. Accounting attempt `b1908d7` is incomplete because it changed only `docs/plan/CHECKLIST.md`, not both owned planning artifacts. The C21 review-fix state has prepared the required two-artifact corrective accounting across `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`; C20 remains pending until that correction is committed and its hash is recorded later without self-reference. No product code is implemented by C20.
...
- [ ] After the planning commit exists, record its hash, exact paths, and review disposition in both durable artifacts and create the separate accounting commit. `b1908d7` attempted this but updated only the checklist; the required two-artifact corrective accounting is now prepared in `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`, but this row remains open until that correction is committed. The corrective commit is not required to contain its own hash.
...
- [ ] Accounting attempts: `b1908d7` (`docs(plan): record host-store adjudication commit`) changed exactly `docs/plan/CHECKLIST.md` and is incomplete because the frozen contract requires both `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`. The corrective accounting is prepared across those exact two required paths; its hash remains intentionally unavailable until creation, and a later C21/C22 accounting commit records it without self-amendment.
...
- [x] OS-read-only-root regression observed non-skipped on Linux: exact command `pnpm exec vitest run tests/service.spec.ts -t "enforces the complete service matrix on a private read-only tmpfs mount"`; result 1/1 passed. Same-identity direct Node write and mkdir sentinels failed with `EROFS` before service construction; absent, incomplete, initialized/fresh, initialized/missing-index, and initialized/stale-index scenarios completed; byte/path trees stayed unchanged. The mandatory proof no longer silently skips: unsupported Linux namespace/mount capability fails explicitly as `C21 required private read-only mount proof is blocked`. The read-only sentinel's unhandled-rejection race was fixed, and namespace teardown removed the private mount, observed the host mountpoint empty, and removed the probe directory in `finally`.
- [x] Durable observed review-fix gate evidence is recorded. The relative-root adapter regression passed 1/1; the mandatory read-only proof passed non-skipped 1/1; the full service/plugin rerun passed 89/89; `pnpm run typecheck` passed; full changed-file ESLint passed with zero warnings; and the diff check passed. These results supersede the pre-review-fix verification state for the affected gates while preserving the previously recorded C21 build, E2E, determinism, pack, sandbox, cleanup, and secure-default evidence.

### C21 review and accounting

- [x] Independent contract review completed with findings: relative-root capture was not exercised through every adapter; C20/C21 ledgers under-accounted existing commits and paths; observed command evidence was not durable. Implemented resolutions add the relative-root adapter regression, prepare C20's two-artifact corrective accounting, record `81f63bd` with its exact eight paths, and preserve durable observed gate evidence.
- [x] Independent security/operations review completed with one finding: the required OS-read-only proof could silently skip on unsupported namespace/mount capability. The implemented resolution makes the mandatory proof fail explicitly as blocked rather than return green, and fixes the read-only sentinel unhandled-rejection race.
- [x] Resolve every finding and rerun affected focused gates before commits are accepted. The relative-root adapter regression and mandatory non-skipping read-only proof each passed 1/1; the full service/plugin rerun passed 89/89; typecheck and full changed-file lint passed; and the diff check passed.
...
- [ ] C21 review-fix/accounting commit: hash intentionally pending creation. Current review-fix accounting covers the prepared C20 two-artifact correction plus C21 tracker/test resolutions; the later ledger must record the actual commit hash and exact path set without requiring the commit to contain its own hash. Final independent re-review and **CLEAN** acceptance also remain pending until after that commit.
