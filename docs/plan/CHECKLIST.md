...
- [x] Exercise the adapter matrix explicitly: tool mutation/read uses agents with distinct session cwd values; commands dispatch through distinct command agents; a direct service caller observes the same captured root. Add a sequential two-activation same-root regression that writes through one activation, reads/lists through the other, disposes/remounts, and confirms durable sharing. Writers remain sequential. A separately mounted distinct root is the isolation negative control.
...
- [x] Composed DSH sandbox/read-only positive-control regression verified: `tests/built-package.e2e.spec.ts` used the pinned disposable `@deepseek-ai/dsh@0.1.1-rc.2` runner/profile and exact fixture lock, asserted the mounted sandbox-policy, fs-sandbox, sandbox, filesystem, and stock-write capabilities at runtime, required the governed stock write to return `FS_SANDBOX_DENIED` without creating its sentinel, and proved a direct `ctx.llmwiki.addSource` host-store write succeeds in the same writable project/profile/process. The composed-policy test passed 1/1; the environment blocker is cleared by this behavioral proof.
- [x] OS-read-only-root regressions verified: `tests/service.spec.ts` created all fixtures inside a disposable Linux private mount namespace and tmpfs, remounted it read-only, required same-identity direct Node write and mkdir sentinels to fail exactly with `EROFS` before service construction, exercised absent, incomplete, initialized/fresh, initialized/missing-index, and initialized/stale-index behavior across every required service surface, and required byte/path snapshots to remain unchanged with no host mount or fixture residue. The private read-only tmpfs test passed 1/1; the environment blocker is cleared by this behavioral proof.
- [x] Focused and packaging verification passed: ESLint passed for changed TypeScript files; `pnpm run typecheck` passed; the focused service+plugin suite passed 89/89; built-package E2E passed 5/5 across its matrix; the private read-only tmpfs test passed 1/1; the composed-policy test passed 1/1; build and determinism audit passed with SHA-256 `f4b636ac0401093955bf3938ec8616014da149dc30b5707a9980cac82781fd12`; `pnpm pack --dry-run` passed and included the empty `cordis.patch.yml`; structured npm pack produced exactly `evegoodevening-dsh-llmwiki-0.1.2.tgz` with 38 files including the patch, README, and package manifest; `git diff --check` passed. C19B was not run or modified and no agent-smoke evidence artifact was created.
