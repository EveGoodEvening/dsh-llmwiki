...
**Status:** draft and review-fix commits recorded; verified uncommitted final service/test fixes complete; independent static re-review and final commit remain open
...
- [x] Focused service/lint/plugin tests passed 113/113; `pnpm run typecheck`, `pnpm run lint`, and `pnpm run build` passed; Loader plus packed/profile E2E passed 8/8; dependency audit reported no known vulnerabilities; no package or lockfile changed.
- [ ] Independent static re-review confirms §14.3 and the current uncommitted final service/test fixes are implemented without an alternate cursor/API/config convention and catalogs/lint remain deterministic/model-free.
- [ ] Commit the verified C16 final service/test fixes only.

**Recorded paths and remaining fixes:** `6516d17` contains exactly 18 paths: `README.md`, `docs/plan/CHECKLIST.md`, `docs/plan/PLAN.md`, `examples/README.md`, `scripts/check-determinism.ts`, `scripts/smoke.ts`, `src/errors.ts`, `src/lint.ts`, `src/presentation.ts`, `src/prompt.ts`, `src/service.ts`, `src/tools.ts`, `src/types.ts`, `tests/built-package.e2e.spec.ts`, `tests/lint.spec.ts`, `tests/loader.e2e.spec.ts`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`. `e50f45b` has actual subject `fix(catalog): validate complete listings` and exactly eight paths: `README.md`, `docs/plan/CHECKLIST.md`, `docs/plan/PLAN.md`, `src/service.ts`, `tests/built-package.e2e.spec.ts`, `tests/loader.e2e.spec.ts`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`. Current verified but uncommitted final changes are limited to `src/service.ts` and `tests/service.spec.ts`: bounded traversal with one directory handle at a time; local ownership and cleanup of `Dir`/`FileHandle` resources; $O(N)$ metadata work plus one page of content bytes; Linux, macOS, and BSD descriptor aliases with fail-closed behavior on unsupported platforms; source error classification; and a self-contained declaration-platform type.
...
- [ ] C16 — draft `6516d17` (`feat(catalog): add deterministic wiki listings`) recorded with exactly 18 paths; review-fix commit `e50f45b` has actual subject `fix(catalog): validate complete listings` and exactly eight paths: `README.md`, `docs/plan/CHECKLIST.md`, `docs/plan/PLAN.md`, `src/service.ts`, `tests/built-package.e2e.spec.ts`, `tests/loader.e2e.spec.ts`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`; verified uncommitted final service/test fixes are complete; gates are focused 113/113, typecheck, lint, build, E2E 8/8, and dependency audit with no known vulnerabilities, with no package or lockfile changes; independent static re-review and final commit remain open
