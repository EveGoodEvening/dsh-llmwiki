...
### Final-gate verification and accounting

- [x] Record coverage-regression commit `5a38926` with actual subject `test(service): cover catalog error boundaries` as a separate final-gate test commit outside all C14–C19A chunk product-commit/path accounting.
- [x] Record final-gate evidence: ordinary unit tests passed 245/245; the C19A-focused suite passed 51/51; typecheck, lint, build, smoke, determinism, and E2E passed, with E2E 8/8.
- [x] Record final coverage: aggregate 93.22% statements, 88.55% branches, 97.59% functions, and 95.75% lines; `src/service.ts` 90.89% statements, 85.33% branches, 95.18% functions, and 94.56% lines.
- [x] Record dependency audit result: no vulnerabilities.
- [x] Preserve closure boundaries: C19B remains externally blocked and unchecked under the exact conditions below; `GAP-SCHEMA` remains intentionally unresolved.
...
- [ ] `DEEPSEEK_API_KEY` is non-empty and authorized; `LLMWIKI_AGENT_SMOKE_MODEL` explicitly names a safe DeepSeek model successfully invokable through pinned DSH agent `0.1.1-rc.2`; `LLMWIKI_AGENT_SMOKE_NETWORK=allow` is explicitly set and outbound access to the configured DeepSeek endpoint is permitted; `pnpm run smoke:agent -- --preflight` exits zero. Blocker: an authorized non-empty `DEEPSEEK_API_KEY`, a safe explicit `LLMWIKI_AGENT_SMOKE_MODEL` successfully invokable through pinned DSH agent `0.1.1-rc.2`, `LLMWIKI_AGENT_SMOKE_NETWORK=allow` with permitted outbound access to the configured DeepSeek endpoint, and zero-exit `pnpm run smoke:agent -- --preflight` are not available as a complete set; preserve the exact applicable probe classification (`BLOCKED_MISSING_CREDENTIAL` or `BLOCKED_NETWORK_NOT_OPTED_IN`).
...
- [ ] Run exactly `pnpm run smoke:agent`; require a real DSH agent/model turn and zero exit. Blocker: an authorized non-empty `DEEPSEEK_API_KEY`, a safe explicit `LLMWIKI_AGENT_SMOKE_MODEL` successfully invokable through pinned DSH agent `0.1.1-rc.2`, `LLMWIKI_AGENT_SMOKE_NETWORK=allow` with permitted outbound access to the configured DeepSeek endpoint, and zero-exit `pnpm run smoke:agent -- --preflight` are not available as a complete set.
- [ ] Prove the agent catalogs and reads existing pages/sources before maintenance, identifies the seeded stale conclusion/contradiction in a separately visible semantic-review result, and names affected page/source IDs. Blocker: an authorized non-empty `DEEPSEEK_API_KEY`, a safe explicit `LLMWIKI_AGENT_SMOKE_MODEL` successfully invokable through pinned DSH agent `0.1.1-rc.2`, `LLMWIKI_AGENT_SMOKE_NETWORK=allow` with permitted outbound access to the configured DeepSeek endpoint, and zero-exit `pnpm run smoke:agent -- --preflight` are not available as a complete set.
- [ ] Independently inspect durable bytes: the affected page is updated or preserves an explicit disagreement with both source IDs; the linked affected page is maintained; no silent schema rewrite occurs. Blocker: an authorized non-empty `DEEPSEEK_API_KEY`, a safe explicit `LLMWIKI_AGENT_SMOKE_MODEL` successfully invokable through pinned DSH agent `0.1.1-rc.2`, `LLMWIKI_AGENT_SMOKE_NETWORK=allow` with permitted outbound access to the configured DeepSeek endpoint, and zero-exit `pnpm run smoke:agent -- --preflight` are not available as a complete set.
- [ ] Start a fresh session and prove catalog/search/read recover the prior state; inspect deterministic structural lint separately from semantic commentary. Blocker: an authorized non-empty `DEEPSEEK_API_KEY`, a safe explicit `LLMWIKI_AGENT_SMOKE_MODEL` successfully invokable through pinned DSH agent `0.1.1-rc.2`, `LLMWIKI_AGENT_SMOKE_NETWORK=allow` with permitted outbound access to the configured DeepSeek endpoint, and zero-exit `pnpm run smoke:agent -- --preflight` are not available as a complete set.
- [ ] Commit sanitized `tests/fixtures/agent-smoke/latest.json` containing schema version, provider/model, UTC timestamps, package/tarball hash, scenario ID, ordered assertion results, safe tool names, durable IDs/hashes, lint counts, and overall result; confirm it contains no prompt/completion, source/page content, credential/header, or absolute path. Blocker: an authorized non-empty `DEEPSEEK_API_KEY`, a safe explicit `LLMWIKI_AGENT_SMOKE_MODEL` successfully invokable through pinned DSH agent `0.1.1-rc.2`, `LLMWIKI_AGENT_SMOKE_NETWORK=allow` with permitted outbound access to the configured DeepSeek endpoint, and zero-exit `pnpm run smoke:agent -- --preflight` are not available as a complete set.
- [ ] Clean disposable profile/root and independently review evidence. C19B completion requires all assertions and alone supplies behavioral closure for `GAP-INGEST`, `GAP-SEMANTIC-LINT`, and `GAP-MODEL-E2E`; mocks, direct tool calls, self-report, or keyless skip never qualify. Blocker: an authorized non-empty `DEEPSEEK_API_KEY`, a safe explicit `LLMWIKI_AGENT_SMOKE_MODEL` successfully invokable through pinned DSH agent `0.1.1-rc.2`, `LLMWIKI_AGENT_SMOKE_NETWORK=allow` with permitted outbound access to the configured DeepSeek endpoint, and zero-exit `pnpm run smoke:agent -- --preflight` are not available as a complete set.
...
**Status:** complete. Planning commit `d0eb13d` is review-clean. Incomplete accounting attempt `b1908d7` changed only `docs/plan/CHECKLIST.md`. Corrective accounting commit `c311a1b` changed exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`, satisfying the two-artifact contract and fully accounting C20. No product code was implemented by C20.
...
- [x] After the planning commit existed, record its hash, exact paths, and review disposition in both durable artifacts and create the separate accounting commit. `b1908d7` attempted this but updated only `docs/plan/CHECKLIST.md`; corrective commit `c311a1b` subsequently changed exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`, recorded the prior accounting accurately, and completed the required two-artifact C20 accounting.
...
- [x] Accounting history: `b1908d7` (`docs(plan): record host-store adjudication commit`) changed exactly `docs/plan/CHECKLIST.md` and is incomplete because the frozen contract requires both durable planning artifacts.
- [x] Corrective accounting commit: `c311a1b`; exact paths are `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`. C20 is fully accounted.
...
- [x] Add one prominent “Storage scope and DSH policy boundary” section to README and the essential warning beside example Loader/root instructions.
- [x] State that the fixed resolved root is durable repository identity; activation owns only cache/initialization/queue state. A relative root is captured from host cwd once, an absolute root is fixed directly, and caller session/workspace cwd is ignored.
- [x] State that separate contexts/processes isolate only with distinct roots; reusing a root shares durable data; concurrent writers in separate activations/processes are unsupported; mutually untrusted tenants must not share an activation or root.
- [x] Enumerate the non-applied seams accurately: `ctx.fs`, filesystem intent/observation events, `DSH_PERMISSION_MODE`, fs-sandbox, read-before-edit, workspace/remote providers, and `ctx.approval`.
- [x] Correct statements implying ordinary tool approval technically gates persistence; retain explicit user authorization solely as agent workflow guidance. The documentation audit asserts the exact warning that a custom `tools/pre-execute` guard gates only selected tool calls and does not cover direct service calls, commands, initialization, or derived-index writes.
- [x] Enumerate every write-capable and initialization-capable surface and document OS-read-only behavior precisely: status/list/lint are strictly non-mutating; source/page mutation and initialization write; `readSource`, `readPage`, and search may initialize root directories/schema and are read-only-usable only on a fully initialized repository; search is additionally conditionally index-publishing, remains read-only with a prebuilt fresh index, and fails without publication when initialization or missing/stale-index rebuild cannot write; `/wiki reindex` writes. Do not call all reads non-mutating and do not claim an acquire-only implementation change.
- [x] Remove the unconditional llmwiki Loader row from the bundled profile and document explicit operator opt-in with an explicit root. Packed-profile coverage now distinguishes installed/default-disabled from explicitly enabled state.
- [x] Add owned README upgrade/release guidance and example guidance for the activation cutover, retaining the shared root, preferring an absolute host-state path, isolating trusted projects, and not opting in where hard read-only/tenant/provider policy is required. State there is no data migration and prohibit automatic copy/split/discovery/deletion.
- [x] Extend the existing documentation audit rather than creating a second convention.

### Tests and focused verification

- [x] Retain the focused service regression proving activation-cwd capture and later `process.cwd()` changes do not move storage.
- [x] Exercise the adapter matrix explicitly with a relative root resolved from a controlled activation cwd, then diverge both `process.cwd()` and caller cwd: tool mutation/read uses agents with distinct session cwd values; commands dispatch through distinct command agents; a direct service caller observes the original absolute captured root; the later process-relative location remains absent. Exact observed command: `pnpm exec vitest run tests/plugin.spec.ts -t "captured root"`; result 1/1 passed. Sequential two-activation same-root sharing, dispose/remount durability, and a distinct absolute-root isolation negative control remain covered with sequential writers.
- [x] Prove the secure default in packed-profile coverage: installation without explicit opt-in exposes no llmwiki service/tools/command/prompt or root, while an explicit operator patch mounts the configured absolute host root and remount persistence survives package removal/re-add.
- [x] Composed DSH sandbox/read-only positive-control regression observed non-skipped: exact command `pnpm exec vitest run --config vitest.e2e.config.ts tests/built-package.e2e.spec.ts -t "composed read-only policy keeps llmwiki host-store writes outside the pinned filesystem policy"`; result 1/1 passed. Runtime evidence required the pinned disposable `@deepseek-ai/dsh@0.1.1-rc.2` profile, mounted sandbox-policy/fs-sandbox/sandbox/filesystem capabilities, stock write denial `FS_SANDBOX_DENIED`, absent stock sentinel, and a successful direct `ctx.llmwiki.addSource` host-store write. Test-owned pack/profile/project/home/wiki roots were removed by registered cleanup callbacks; no fixture or tarball remained.
- [x] OS-read-only-root regression observed non-skipped on Linux: exact command `pnpm exec vitest run tests/service.spec.ts -t "private read-only tmpfs"`; result 1/1 passed. Same-identity direct Node write and mkdir sentinels failed with `EROFS` before service construction; absent, incomplete, initialized/fresh, initialized/missing-index, and initialized/stale-index scenarios completed; byte/path trees stayed unchanged. The mandatory proof is Linux-scoped: non-Linux hosts do not register it, while unsupported Linux namespace/mount capability fails explicitly as `C21 required private read-only mount proof is blocked` rather than reporting a green capability skip. The read-only sentinel's unhandled-rejection race was fixed, namespace teardown removed the private mount, and commit `164bed6` contains exactly `tests/service.spec.ts`. Exact observed lint command after that scope fix: `pnpm exec eslint tests/service.spec.ts --max-warnings 0`; result passed with zero warnings.
- [x] Durable observed review-fix gate evidence is recorded with exact invocations and results: relative-root adapter `pnpm exec vitest run tests/plugin.spec.ts -t "captured root"` — 1/1 passed; full service/plugin rerun `pnpm exec vitest run tests/service.spec.ts tests/plugin.spec.ts` — 89/89 passed; changed-file lint `pnpm exec eslint scripts/check-determinism.ts tests/service.spec.ts tests/plugin.spec.ts tests/built-package.e2e.spec.ts --max-warnings 0` — passed with zero warnings; diff check `git diff --check` — passed with no output; Linux-scope proof `pnpm exec vitest run tests/service.spec.ts -t "private read-only tmpfs"` — 1/1 passed; Linux-scope lint `pnpm exec eslint tests/service.spec.ts --max-warnings 0` — passed with zero warnings. These results supersede the pre-review-fix verification state for the affected gates while preserving the previously recorded C21 build, E2E, determinism, pack, sandbox, cleanup, and secure-default evidence.

### C21 review and accounting

- [x] Independent contract review completed with findings: relative-root capture was not exercised through every adapter; C20/C21 ledgers under-accounted existing commits and paths; observed command evidence was not durable. Implemented resolutions add the relative-root adapter regression, establish the current exact-two-artifact C20 corrective accounting change, record `81f63bd` with its exact eight paths, record `fb579f2` with its exact four paths, and preserve exact observed commands/results.
- [x] Independent security/operations review completed with findings: the required OS-read-only proof could silently skip on unsupported Linux namespace/mount capability, its sentinel promises could race, and the initial non-skipping fix registered a Linux-specific fixture on non-Linux hosts. `fb579f2` made Linux capability failure explicit and fixed the sentinel race; `164bed6` restored non-Linux exclusion while retaining mandatory failure on Linux.
- [x] Resolve the recorded implementation/evidence findings and rerun affected focused gates before commits are accepted. Exact observed results: `pnpm exec vitest run tests/plugin.spec.ts -t "captured root"` — 1/1 passed; `pnpm exec vitest run tests/service.spec.ts tests/plugin.spec.ts` — 89/89 passed; `pnpm exec eslint scripts/check-determinism.ts tests/service.spec.ts tests/plugin.spec.ts tests/built-package.e2e.spec.ts --max-warnings 0` — passed with zero warnings; `git diff --check` — passed with no output; `pnpm exec vitest run tests/service.spec.ts -t "private read-only tmpfs"` — 1/1 passed; `pnpm exec eslint tests/service.spec.ts --max-warnings 0` — passed with zero warnings.
- [x] Commit implementation: `81f63bd` (`docs: document opt-in process-wide host wiki storage`). Its checklist mutation was an ownership deviation from the accounting-only rule and is recorded in the actual eight-path set rather than omitted. Review fixes belong in a separately named commit and are accounted only after creation.
- [x] C21 implementation commit: `81f63bd`; exact eight paths: `README.md`, `cordis.patch.yml`, `docs/plan/CHECKLIST.md`, `examples/README.md`, `scripts/check-determinism.ts`, `tests/built-package.e2e.spec.ts`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`.
- [x] C21 review-fix commits recorded: `fb579f2` (`test: harden host-store boundary evidence`) changed exactly `docs/plan/PLAN.md`, `docs/plan/CHECKLIST.md`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`; `164bed6` is the Linux-scope review-fix commit and changed exactly `tests/service.spec.ts`. Neither commit is the required exact-two-path C20 corrective accounting commit.
- [x] C21 final acceptance: three independent final re-reviews unanimously returned **CLEAN** with no findings. All prior functional, security, evidence, cleanup, and accounting findings are resolved. `c311a1b` is recorded as the exact-two-artifact corrective C20 accounting commit; `81f63bd`, `fb579f2`, and `164bed6` are recorded with their exact path sets. C21 implementation, verification, review, commits, and accounting are complete. This final accounting commit does not record its own hash under the no-self-hash rule.
...
**IDs repository-resolved by C21 but GitHub-closed only after each issue's C22 approved-comment and supported-reason closure milestones both pass:** `ISSUE-2-WORKSPACE-SCOPE`, `ISSUE-3-FILESYSTEM-POLICY` \
**Owned paths:** `docs/plan/PLAN.md`, `docs/plan/CHECKLIST.md`; necessary C21 corrections must precede and be separately accounted \
**Status:** repository-local C22 work is complete. Review A's sole initial finding was fixed by `9c7cf07`; distinct adversarial source-ID hardening was completed by `e9f504f`; Review B and final split/aggregate re-reviews were unanimously **CLEAN**; all previously recorded repository-local gates passed. Pre-closure commit `fc4897c` changed exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`. The durable non-mutating preflight completed state reads and close-reason support checks, but authenticated identity/authorization failed: identity `null`, `GET /user` HTTP 401, no `gh`/token/enabled GitHub MCP/authenticated relay, and `can_close=false`. External mutation is blocked, not deferred: `BLOCKED-GITHUB-CLOSURE: no authenticated GitHub identity or Issues: write owner/push/Triage authorization is available`. C19B and `GAP-SCHEMA` remain unchanged.

### Review A — scope, identity, and compatibility

- [x] Inspect source, built entry, adapters, tests, public docs, and durable plan. Confirm fixed resolved root—not activation—is repository identity; activation state/queue is local; distinct roots isolate; reused roots share data; separate-process writers are unsupported.
- [x] Confirm relative/absolute root semantics, explicit opt-in, adapter matrix, no-data-migration compatibility, and multi-tenant limitation are identical across artifacts.
- [x] Record findings and their correction disposition: Review A found only that the command adapters' status/lint success assertions did not prove captured-root routing; `9c7cf07` directly fixed that finding with root-dependent exact output assertions. A distinct later adversarial command re-review required an independently pinned SHA-256 source ID, supplied as follow-up hardening by `e9f504f`, not attributed as another initial Review A finding. Focused verification passed, every finding is resolved, and the final scope/root/adapter re-review returned **CLEAN**.

### Review B — filesystem, permission, and security ownership

- [x] Inspect production I/O, injection/dependencies, bundled profile composition, tests, and public wording. Confirm exclusions and every write-capable or initialization-capable surface exactly match runtime behavior: status/list/lint are non-mutating; source/page reads and search may initialize; search is also conditionally index-publishing.
- [x] Confirm secure default/explicit opt-in, absent/incomplete/initialized-fresh/initialized-stale OS-read-only behavior, direct Node sentinel denial under the same identity, hard-policy “do not opt in” guidance, owned README upgrade/release guidance, and the custom tool-guard coverage warning.
- [x] Confirm the DSH boundary test uses an actually composed mechanism plus controlled denial; the governed stock write is denied while explicitly opted-in host-store persistence succeeds. No inert permission-mode evidence is used.
- [x] Confirm no closure rationale claims that `ctx.fs` migration is impossible; future capability needs are requirements to source and verify.
- [x] Record Review B split verdict **CLEAN** with no findings after the filesystem/permission/security inspection. The final filesystem/permission/security re-review and aggregate accounting also returned **CLEAN** with zero unresolved findings.

### GitHub capability and authorization preflight

- [x] Non-mutatingly read both issues. Issue #2 is open with `state_reason=null`, `closed_at=null`, zero comments, empty comments/events/timeline results, approved exact comment absent, and canonical URL `https://github.com/EveGoodEvening/dsh-llmwiki/issues/2`. Issue #3 has the same live state with canonical URL `https://github.com/EveGoodEvening/dsh-llmwiki/issues/3`.
- [ ] Identify an authenticated GitHub identity and confirm repository issue-closure permission. Current authenticated identity is `null`; `GET /user` returned HTTP 401; no `gh` installation/stored auth, token, enabled GitHub MCP, or authenticated browser relay is available; owner/push/Triage authorization was not established and `can_close=false`. `BLOCKED-GITHUB-CLOSURE: no authenticated GitHub identity or Issues: write owner/push/Triage authorization is available`.
- [x] Confirm GitHub supports the intended close reason: the update-issue API accepts `state_reason=not_planned` with `state=closed`.
- [x] Review and approve the separate exact closure comments below. Issue #2 cites the continuing claim, fixed-root/adapter evidence, sequential same-root durable sharing, activation-local state, distinct-root isolation, shared-root writer/multi-tenant warnings, no runtime fix, and joint reopen criteria. Issue #3 cites the continuing direct-host-I/O boundary, explicit opt-in/secure default, composed controlled-denial and Linux OS-read-only evidence, excluded policy seams, initialization-capable read/search surfaces, conditional index publication, hard-policy guidance, no runtime fix, and joint reopen criteria.
...
```
- [x] Record the pre-mutation blocked branch without fabricating mutation evidence: `BLOCKED-GITHUB-CLOSURE: no authenticated GitHub identity or Issues: write owner/push/Triage authorization is available`. Both issues remain open and no comment was posted. A safe resume requires an authenticated GitHub session or repository-scoped token with Issues: write plus owner, push, or Triage authority; then rerun and durably record the complete non-mutating identity, permission, issue/comment state, and close-reason preflight before mutation.

### Pre-closure commit, external mutation, and final accounting

- [x] Record final repository-local verification after fixes: `pnpm run lint` passed; `pnpm run typecheck` passed; `pnpm test` passed 7 files/248 tests; `pnpm run test:e2e` passed 2 files/9 tests; `pnpm run build && pnpm run check:determinism` passed with SHA-256 `f4b636ac0401093955bf3938ec8616014da149dc30b5707a9980cac82781fd12`; `pnpm pack --dry-run` passed with artifact `evegoodevening-dsh-llmwiki-0.1.2.tgz`; and `git diff --check` passed. LSP diagnostics were unavailable because no language server was found for `tests/plugin.spec.ts`. No product source/API/dependency/storage-format/default-root-value change or artifact leak was reported; C19B and `GAP-SCHEMA` boundaries remain unchanged.
- [x] Record the blocked pre-closure commit `fc4897c` with exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`. This completes repository-local C22 work and durable blocked-preflight accounting; it does not authorize mutation. A later successful-authorization update commit must precede any comment or close.
- [ ] Only after that successful-authorization commit exists, process each issue as two ordered durable milestones: (1) confirm whether the exact approved comment already exists, posting it only if absent and requiring confirmed success with its URL; (2) only then confirm whether the issue is already closed, closing it only if open with the confirmed supported reason. Never attempt an issue close before its approved comment succeeds. `BLOCKED-GITHUB-CLOSURE: no authenticated GitHub identity or Issues: write owner/push/Triage authorization is available`.
- [ ] After any comment or close failure, immediately inventory both issues' exact milestone states: approved-comment present/absent and URL, plus open/closed state and supported reason. Create a partial-mutation accounting commit that records every confirmed successful action per issue, whether the failure was comment-before-close or close-after-comment, the exact failed/missing action and blocker, the latest successful-authorization/pre-closure commit hash/path set, and the ledger known so far. Do not collapse the two failure states or wait for an all-or-nothing outcome before persisting success. Deferred unless an authorized mutation partially succeeds or fails; if either occurs, this row becomes required.
- [ ] On any authorized resume, read the latest durable partial state, non-mutatingly re-inventory both issues, skip every milestone already confirmed complete, and attempt only remaining milestones in order. Never repost a completed comment or re-close an issue already confirmed closed. If an issue is closed but its approved comment is absent, keep it partial and attempt only the missing comment. Persist another partial-mutation accounting commit after any further failure. `BLOCKED-GITHUB-CLOSURE: no authenticated GitHub identity or Issues: write owner/push/Triage authorization is available`.
- [ ] Close `ISSUE-2-WORKSPACE-SCOPE` in the durable ledger only after issue #2's exact approved comment is confirmed present with its URL and the issue is confirmed closed with the supported reason; apply the same two-milestone rule to `ISSUE-3-FILESYSTEM-POLICY`. Only after both issues satisfy both milestones, record actual resulting URLs/states, the successful-authorization/pre-closure and any partial-accounting commit hashes/path sets, and the complete prior ledger in a later final closure-accounting commit. That final commit does not contain its own hash; report its hash as delivery evidence after creation. `BLOCKED-GITHUB-CLOSURE: no authenticated GitHub identity or Issues: write owner/push/Triage authorization is available`.
- [x] Preserve the safe, idempotent blocked-resume sequence: obtain authenticated Issues: write owner/push/Triage authority; rerun the full non-mutating preflight; commit the successful authorization update; for each issue, inventory the approved exact comment and post only if absent, confirm its URL, close only if still open with `state_reason=not_planned`, and re-read state/comments; skip confirmed milestones; after any failure inventory both issues and commit all confirmed partial success before another resume; complete decision-ID and final closure accounting only after both comments and both closures are confirmed.

### C22 accounting

- [x] Review A verdict/findings: the scope/root/adapter review found only that command status/lint success assertions did not prove captured-root routing. `9c7cf07` directly fixed that finding by making both command agents assert exact captured-repository status/lint output. A distinct later adversarial command re-review required the expected SHA-256 source ID to be independently pinned across tool, command, and direct-service observations; `e9f504f` records that follow-up hardening rather than an additional initial Review A finding. The corrected attribution and every disposition are complete; final split and aggregate re-reviews returned unanimous **CLEAN** with zero unresolved findings.
- [x] Review B verdict/findings: filesystem/permission/security split review and final re-review **CLEAN**, with no findings. They verified direct Node host-storage ownership, empty bundled patch and explicit opt-in, composed-policy controlled denial, Linux OS-read-only cases, exact initialization-capable read/search and conditional-index semantics, excluded policy seams, cleanup, and C19B separation. Aggregate independent review is complete and **CLEAN**.
- [x] Final verification: focused corrections remain recorded for `9c7cf07` and `e9f504f`, each changing exactly `tests/plugin.spec.ts` and passing the focused captured-root test 1/1 plus eslint/typecheck. Final observed gates passed exactly: `pnpm run lint`; `pnpm run typecheck`; `pnpm test` (7 files/248 tests); `pnpm run test:e2e` (2 files/9 tests); `pnpm run build && pnpm run check:determinism` with SHA-256 `f4b636ac0401093955bf3938ec8616014da149dc30b5707a9980cac82781fd12`; `pnpm pack --dry-run` with `evegoodevening-dsh-llmwiki-0.1.2.tgz`; and `git diff --check`. LSP diagnostics were unavailable because no language server was found for `tests/plugin.spec.ts`.
- [x] GitHub non-mutating state/read/reason-support preflight: issues #2/#3 are open with `state_reason=null`, `closed_at=null`, comments/events/timeline empty, and approved exact comments absent; canonical URLs are recorded above; `not_planned` is supported.
- [ ] GitHub authenticated identity/capability/authorization: identity `null`; `GET /user` HTTP 401; no `gh`/token/enabled GitHub MCP/authenticated relay; `can_close=false`. `BLOCKED-GITHUB-CLOSURE: no authenticated GitHub identity or Issues: write owner/push/Triage authorization is available`.
- [x] Blocked pre-closure status/commit: repository-local work complete in `fc4897c`, which changed exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`; external mutation is blocked, not deferred.
- [x] Issue #2 approved comment: exact text durably approved; SHA-256 `3eed98911b6c5e9b1aa7277b086c748a7f296367d488e758caffd977c0f431a0`.
- [ ] Issue #2 GitHub closure milestone: live read shows open, `state_reason=null`, approved comment absent and therefore no comment URL; posting, closing, resulting URL/closed state, and `ISSUE-2-WORKSPACE-SCOPE` GitHub closure remain unchecked. `BLOCKED-GITHUB-CLOSURE: no authenticated GitHub identity or Issues: write owner/push/Triage authorization is available`.
- [x] Issue #3 approved comment: exact text durably approved; SHA-256 `3ce1a464ae939bb9800c73460528110b81dfd64e73b0a61bc5210c841112a6ae`.
- [ ] Issue #3 GitHub closure milestone: live read shows open, `state_reason=null`, approved comment absent and therefore no comment URL; posting, closing, resulting URL/closed state, and `ISSUE-3-FILESYSTEM-POLICY` GitHub closure remain unchecked. `BLOCKED-GITHUB-CLOSURE: no authenticated GitHub identity or Issues: write owner/push/Triage authorization is available`.
- [ ] Partial-mutation accounting commit(s): none because no external mutation was attempted. Deferred unless an authorized mutation partially succeeds or fails; if either occurs, this row becomes required and exact paths must be the two durable planning files.
- [ ] Final closure-accounting commit: `<pending-delivery-evidence>`; create only after both issues have their exact approved comment confirmed and their supported-reason closure confirmed; exact paths must be the two durable planning files; never self-record its hash. `BLOCKED-GITHUB-CLOSURE: no authenticated GitHub identity or Issues: write owner/push/Triage authorization is available`.

## Issues #2/#3 commit ledger

- [x] C20 — planning commit `d0eb13d` changed exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`; incomplete attempt `b1908d7` changed exactly `docs/plan/CHECKLIST.md`; corrective accounting commit `c311a1b` changed exactly `docs/plan/PLAN.md` and `docs/plan/CHECKLIST.md`. Review disposition is CLEAN and C20 is fully accounted.
- [x] C21 — implementation commit `81f63bd` changed exactly `README.md`, `cordis.patch.yml`, `docs/plan/CHECKLIST.md`, `examples/README.md`, `scripts/check-determinism.ts`, `tests/built-package.e2e.spec.ts`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`; review-fix commit `fb579f2` changed exactly `docs/plan/PLAN.md`, `docs/plan/CHECKLIST.md`, `tests/plugin.spec.ts`, and `tests/service.spec.ts`; Linux-scope review-fix commit `164bed6` changed exactly `tests/service.spec.ts`. Focused command/result evidence, secure-default cutover, cleanup, and three unanimous final independent **CLEAN** re-reviews are recorded. All C21 implementation, verification, review, commit, and accounting rows are complete. `ISSUE-2-WORKSPACE-SCOPE` and `ISSUE-3-FILESYSTEM-POLICY` are repository-resolved with no runtime fix required, but are not GitHub-closed.
