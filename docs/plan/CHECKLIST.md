...
**Status:** planned and unchecked. Both decision IDs are repository-resolved with no runtime fix required, but neither GitHub issue is claimed closed. GitHub review, preflight, authorization, comments, mutation, and closure accounting occur only in C22 after the latest pre-closure/authorization-update commit durably records a successful preflight; a durable blocked result never authorizes mutation.
